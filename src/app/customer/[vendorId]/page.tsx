...
interface DisplayItem { item_name: string; quantity: number; unit_price: number; subtotal: number; }
interface DisplayOrder { id: number; order_number: string; total: number; bill_url?: string; items: DisplayItem[]; }
...
export default function VendorCustomerPage() {
  ...
  /* fetch shop name */
  useEffect(() => {
    fetch(`/api/customer/vendor/${vendorId}`)
  ...
  useEffect(() => {
    async function poll() {
      ...
      try {
        const res = await fetch(`/api/customer/status?vendorId=${vendorId}`);
      ...
      if (data.status === 'paid' && data.order) {
        if (cur !== 'paid') {
          ...
          updateStatus('paid'); setOrder(data.order);
  ...
  return (
    <div className="min-h-screen bg-[#020817] text-white flex flex-col overflow-hidden select-none">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        {status === 'paid' && <div className="absolute inset-0 bg-emerald-500/[0.04] transition-opacity duration-1000" />}
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between px-7 py-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={toggleFullscreen} className="p-1.5 rounded-lg text-white/20 hover:text-white/50 transition-colors">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-6 py-4">
        {status === 'idle'  && <IdleScreen time={time} shopName={shopName} />}
        {isActive && order  && <ActiveOrderScreen order={order} showQr={status === 'pending'} />}
        {status === 'paid' && order && <PaidScreen order={order} />}
      </div>

      <div className="relative z-10 flex items-center justify-center pb-5 flex-shrink-0">
        <p className="text-[11px] text-white/10 tracking-widest uppercase">Powered by Velora</p>
      </div>
    </div>
  );
}
...
/* ── Idle ─────────────────────────────────────────────────────────────── */
function IdleScreen({ time, shopName }: { time: string; shopName: string }) {
  return (
    <div className="flex flex-col items-center text-center">
      <p className="text-7xl sm:text-8xl font-extralight tabular-nums text-white/30 tracking-wider mb-14">{time}</p>
      ...
      <h1 className="text-5xl sm:text-6xl font-black tracking-[0.18em] text-white/90 mb-3">
        {shopName ? shopName.toUpperCase() : 'VELORA'}
      </h1>
      <p className="text-sm text-white/25 tracking-[0.25em] uppercase mb-14">Welcome</p>
      ...
      <div className="flex-1 flex flex-col bg-white/[0.03] border border-white/[0.07] rounded-3xl overflow-hidden backdrop-blur-sm">
        <div className="px-7 py-5 border-b border-white/[0.06]">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-semibold text-white/30 uppercase tracking-[0.2em] mb-1">Your Order</p>
              <p className="text-lg font-bold text-white/80">{order.order_number}</p>
            </div>
            {showQr
              ? <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-xs font-semibold text-indigo-400"><span className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse" />Payment Ready</span>
              : <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-xs font-semibold text-amber-400"><span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />Preparing</span>
            }
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {order.items.length === 0
            ? <div className="flex items-center justify-center py-12 text-white/20 text-sm">Items will appear here as they&apos;re added…</div>
            : <div className="divide-y divide-white/[0.05]">
                {order.items.map((item, i) => (
                  <div key={i} className="flex items-center justify-between px-7 py-4">
                    <div className="flex items-center gap-4 min-w-0">
                      <span className="w-8 h-8 rounded-full bg-indigo-600/20 border border-indigo-500/20 text-indigo-400 text-sm font-bold flex items-center justify-center flex-shrink-0">{item.quantity}</span>
                      <span className="text-base text-white/80 font-medium truncate">{item.item_name}</span>
                    </div>
                    <span className="text-base font-semibold text-white/60 ml-4 flex-shrink-0">RM {Number(item.subtotal).toFixed(2)}</span>
                  </div>
                ))}
              </div>
          }
        ...
        {showQr && order.bill_url ? (
          <div className="flex flex-col items-center gap-6 w-full">
            <div className="text-center">
              <p className="text-[10px] font-semibold text-white/25 uppercase tracking-[0.2em] mb-2">Amount Due</p>
            ...
            <div className="text-center space-y-1.5">
              ...
              <div className="flex items-center justify-center gap-2">
                {['DuitNow', 'FPX', 'Online Banking'].map(m => (
                  <span key={m} className="px-2.5 py-0.5 rounded-full border border-white/10 text-xs text-white/30">{m}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-8 text-center">
            <div className="relative w-28 h-28">
              ...
              <div className="absolute inset-0 flex items-center justify-center"><Zap size={32} className="text-indigo-400/60" /></div>
            </div>
            <div>
            ...
            <div className="flex gap-2">
              {[0,1,2].map(i => <span key={i} className="w-2 h-2 rounded-full bg-indigo-500/40 animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />)}
            </div>
          </div>
        )}
      </div>
...
function PaidScreen({ order }: { order: DisplayOrder }) {
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <h1 className="text-6xl sm:text-7xl font-black text-white tracking-tight mb-3">Thank You!</h1>
      <p className="text-lg text-white/30 mb-8 font-light tracking-wide">Payment Successful</p>
      <div className="px-10 py-5 rounded-3xl bg-emerald-500/5 border border-emerald-500/10 mb-8">
        <p className="text-5xl font-black text-emerald-400">RM {Number(order.total).toFixed(2)}</p>
      </div>
