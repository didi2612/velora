    ...
    <div className="space-y-5">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-500">
            ...
            {search === '' && catFilter === 'All' && (
              <button onClick={openAdd} className="mt-3 text-indigo-400 hover:text-indigo-300 text-sm">
                Add your first item
              </button>
            )}
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700 bg-slate-800/50">
                  ...
                  <th className="text-right px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Price</th>
              ...
              <tbody className="divide-y divide-slate-700/60">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-slate-700/20 transition-colors group">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${CATEGORY_COLORS[item.category] ?? CATEGORY_COLORS.General}`}>
                        {item.category}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm font-semibold text-emerald-400">RM {Number(item.price).toFixed(2)}</span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className={`text-sm font-medium ${item.stock <= 5 ? 'text-red-400' : item.stock <= 20 ? 'text-amber-400' : 'text-slate-300'}`}>
                        {item.stock}
                      </span>
                    </td>
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          <button className="flex-1 bg-red-600 hover:bg-red-500 text-white font-medium px-4 py-2 rounded-lg transition-colors" onClick={() => confirmId && handleDelete(confirmId)}>
            Delete
          </button>
        </div>
      </Modal>
