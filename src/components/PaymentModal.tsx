...
interface Props { order: Order | null; onPaid: (order: Order) => void; onClose: () => void; }
...
export default function PaymentModal({ order, onPaid, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (updated.paid) { stopPoll(); setStage('paid'); setBill(updated); setTimeout(() => order && onPaid(order), 1800); }
      } catch { /* ignore */ }
    }, 3000);
    return stopPoll;
  ...
  useEffect(() => {
    if (!order) return;
    setName('Customer'); setStage('name'); setBill(null); stopPoll();
  }, [order, stopPoll]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/75 backdrop-blur-sm" onClick={stage === 'paid' ? undefined : onClose} />
      <div className="relative w-full max-w-sm bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
        <div className="px-6 pt-6 pb-4 text-center border-b border-slate-700">
          ...
          <p className="text-3xl font-black text-emerald-400 mt-1">RM {Number(order.total).toFixed(2)}</p>
        </div>
          ...
          {stage === 'name' && (
            <div className="space-y-4">
              <div>
                ...
                <input className="input" value={name} onChange={e => setName(e.target.value)} placeholder="Customer" autoFocus onKeyDown={e => e.key === 'Enter' && createBill()} />
              </div>
              <button className="btn-primary w-full" onClick={createBill}>Generate Payment QR</button>
          ...
          {stage === 'qr' && bill && (
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="p-3 bg-white rounded-2xl shadow-lg">
                <QRCodeSVG value={bill.url} size={210} level="M" includeMargin={false} />
              </div>
              ...
              <a href={bill.url} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-indigo-500/30 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 text-sm font-medium transition-colors">
                <ExternalLink size={14} /> Open payment page directly
              </a>
              <div className="w-full flex gap-2">
                <button className="flex-1 flex items-center justify-center gap-1.5 text-xs py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 transition-colors" onClick={() => { stopPoll(); setStage('name'); }}>
                  <RefreshCw size={12} /> Regenerate
                </button>
                <button className="flex-1 text-xs py-2 rounded-lg btn-danger" onClick={onClose}>Cancel</button>
              </div>
            </div>
          )}
          {stage === 'error' && (
            <div className="flex flex-col items-center gap-4 py-4">
              ...
              <button className="btn-primary w-full" onClick={() => setStage('name')}>Try Again</button>
          ...
          {stage === 'paid' && (
            <div className="flex flex-col items-center gap-4 py-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-slate-400 mt-1">RM {Number(order.total).toFixed(2)} — {bill?.name}</p>
              </div>
